...
function handleLinkDuplicateError(file, link, allowDeadLinks = false) {
  const { line, column } = start(link.node);
  const notify = allowDeadLinks ? file.message : file.warn;

  notify(`Link is a duplicate: ${link.link.href}`, { line, column });
}
